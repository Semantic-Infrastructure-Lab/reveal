...
### Changed
- **Switch/case arms no longer double-count in complexity (BACK-1289)** — a JS `switch_case` (or Python `case_clause`) and its bare `case` keyword token each counted, and Ruby's `case` container counted on top of every `when`. JS switch with 2 cases: 5 → 3; Ruby case + 2 `when`: 4 → 3; Python match with 2 cases: 6 → 4 (the `match` container still counts, pinned by the taxonomy test). Published complexity and C901 numbers drop for these constructs.
...
- **`ast://` `--calls`/`--sideeffects`/`--boundary` now agree with `calls://` (BACK-1299, part of BACK-1279)** — three nav-side gaps found by a new cross-path agreement test: Ruby paren-less calls (`helper`) were invisible, Java `new Foo()` / `new Box<T>()` were dropped, and every Dart call also emitted a junk `()` callee (Dart `new`/`const`/`Type.named` constructors now resolve too).
