...
- **`--sort`/`--limit` and `?sort=`/`?limit=` broke `env://`, `python://`, `help://`, `cpanel://`, `domain://` and `diff://` (BACK-1385)** — the flag table called them "understood by every adapter", but these six read no result-control keys: the CLI form (`reveal env:// --limit 2`) or a typed `?limit=2` was glued onto the resource and failed as `Element '?limit=2' not found` (or a path error for `diff://`), and `domain://` warned and dropped it. Adapters now declare `HONORS_RESULT_CONTROL`; on those six, `--sort`/`--limit` print `Note: --limit has no effect on env:// ...` and run normally, and a typed `sort=`, `limit=` or `offset=` is `Error: env:// does not support limit= -- remove it from the URI.` (exit 1, JSON error envelope). Other query keys (`help://search?search=`, cpanel's `domain_type`) are untouched, as is every adapter that applies these keys.
