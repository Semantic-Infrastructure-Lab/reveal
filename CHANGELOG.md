...
- **`--limit` answered "Unknown query param 'limit'" on `hotspots://`, `calls://`, `depends://` and `testability://` (BACK-1496)** — each already caps its results with `top=`, but the CLI flag was injected as the universal `limit=` and ignored. `--limit N` now becomes the adapter's own `top=N` (declared in `CLI_QUERY_FLAGS`), and `--limit 0` means no cap (`hotspots` treats a literal `top=0` as "nothing"). `imports://`, `architecture://`, `deps://` and `overview://` keep the warning: their `top` only applies to some views, so mapping it would turn a loud warning into a silent no-op.
- **`--head`/`--tail`/`--range` silently did nothing on results with several lists (BACK-1497)** — `hotspots://` (file + function hotspots) and `reveal://` (analyzers/adapters/rules) came back unchanged. They now slice every top-level list of records and say which (`Note: --head applied to each of ...`); a result with no list prints `Note: --head has no effect on env://` instead of nothing. `--max-items`/`--budget` on such results is unchanged.
