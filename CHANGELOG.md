...
### Changed
- **C901 reports the same complexity as `ast://` and `stats://` for Python too (BACK-1081)** — Python was re-scored with `mccabe`, a second definition that ignores boolean operators and ternaries, so one function could read 30 in `ast://` and 24 in C901, and `mccabe` results were keyed by bare name so a same-named method in another class overwrote them. C901 now uses the analyzer's per-function score for every language (the definition radon, lizard and SonarQube use; `&&`/`||`/`??`/`?:` counted for every language except Scala, BACK-1316/1324), and the `mccabe` dependency is gone. Python C901 numbers now run higher than Ruff's; on reveal's own source about 50 of 3,900 functions cross the default threshold (21) differently. `.reveal.yaml` thresholds tuned against the old numbers may need a nudge.
