...
### Fixed
- **Python type stubs (`.pyi`) had no analyzer (BACK-1467)** — `reveal x.pyi` printed "No analyzer found" although the import extractor already read stubs. Stubs now use the Python analyzer (outline, element extraction, `ast://`). The import rules (I001/I002/I005) run on them; body-level rules and M102 do not, since a stub has no bodies and nothing imports it by path. I001 and `imports://?unused` also now treat a PEP 484 redundant alias (`import X as X`, `from m import X as X`) as an explicit re-export rather than an unused import, in `.py` files as well as stubs, and skip `__init__.pyi` like `__init__.py`.
