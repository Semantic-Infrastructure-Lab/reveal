...
### Fixed
- **C901 no longer loses a Python method's complexity to a same-named method elsewhere in the file (BACK-1081)** — McCabe scores were keyed by bare function name, so with `A.run` (complex) and `B.run` (trivial) in one file the last definition overwrote the first and the complex one was never flagged. Scores are now matched by name and position (decorated functions included).
