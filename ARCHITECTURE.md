...
`_get_parse_cache()` returns a `threading.local()`-scoped `OrderedDict` (max 128 entries per thread), reachable via `_parse_cache_local`. It exists to avoid redundant parses during multi-pass analysis within one thread (e.g., `reveal check` running multiple rules against the same file). It was a single module-level dict shared across all analyzers in a process until BACK-1136: reveal-mcp dispatches each tool call onto its own `anyio` worker thread, and tree-sitter's `Tree`/`Node` objects are pyo3 types that are not `Send` — a second thread touching, overwriting, or garbage-collecting one built by a different thread panics or raises an unraisable `RuntimeError` in the Rust extension, which could kill the whole server process. Making the cache thread-local means no thread ever *explicitly* touches another thread's Tree/Node objects; the trade-off is that cache hits no longer cross threads (each reveal-mcp worker thread rebuilds its own cache for a file the first time it lands on that thread), which only matters for the concurrent MCP-server case — the single-threaded CLI's behavior is unchanged.

**This is a mitigation, not a complete fix — do not read it as "the thread-safety problem is solved."** A thread-local cache stops threads from reaching into each other's cache entries, but it cannot stop CPython's *cyclic garbage collector*, which runs on whichever thread happens to trigger a collection and will reclaim cycles holding `Tree`/`Node` objects created on some other thread. That path violates the same pyo3 `unsendable` contract and is not addressable from reveal's side. Measured directly against the pinned language-pack version (minimal harness, no reveal code): dropping a `Tree` cross-thread and collecting a `Tree`-bearing reference cycle from another thread each raise `RuntimeError: Tree is unsendable, but is being dropped on another thread`, and a concurrent-GC stress loop produces thousands of them plus a **`SIGABRT` hard abort** that cannot be caught from Python (see [PyO3 #3688](https://github.com/PyO3/pyo3/issues/3688), [#5067](https://github.com/PyO3/pyo3/issues/5067)).

The root cause was the `tree-sitter-language-pack>=1.8.1,<1.12.5` pin that used to live in `pyproject.toml`: at or below `1.12.2` the pack supplies a *vendored* pyo3-backed parser whose `Parser`/`Tree` are `#[pyclass(unsendable)]`. Past that pin (`1.12.5`+) `get_parser` hands back the installed **core `tree_sitter` C binding**, which carries no thread-affinity constraint — the same stress test then reports zero errors and exits cleanly. As of `BACK-1161`, that `<1.12.5` ceiling has been dropped from `pyproject.toml` (now `tree-sitter-language-pack>=1.8.1`, paired with a new `tree-sitter<0.26.0` core ceiling for an unrelated refcounting bug), which should resolve this failure class on a fresh install — but it is not yet independently re-verified against this specific crash (`BACK-1146` note #6).
