...
The root cause is the `tree-sitter-language-pack>=1.8.1,<1.12.5` pin in `pyproject.toml`: at or below `1.12.2` the pack supplies a *vendored* pyo3-backed parser whose `Parser`/`Tree` are `#[pyclass(unsendable)]`. Past that pin (`1.12.5`+) `get_parser` hands back the installed **core `tree_sitter` C binding**, which carries no thread-affinity constraint — the same stress test then reports zero errors and exits cleanly. Removing the whole class of failure therefore depends on the core-API migration that pin is holding back, not on further caching changes here.
