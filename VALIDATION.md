...
`package.json` `exports` map resolution), not yet filed as separate tasks.
See the [harness
...
basename-collision false-positive class and 3 unexplained misses, were both
closed in a same-program follow-up (BACK-670, which incidentally also
resolved BACK-671) rather than counted against this headline, matching how
TS's BACK-643 and C++'s BACK-642 were handled. PHP's
