...
| Kotlin | ✅ 99.1% (tivi, 100%¹⁴ kotlinx.coroutines) | ✅ 82.5% → **92.9%** (tivi, six-category sweep, BACK-727) | ✅ 99.69%→100%⁴² (8/bucket); 99.79%⁴² (20/bucket, tivi, BACK-738 shape 1 fixed & shipped in 0.123.0 — shape 5 SAM labeled-return spillage remains an accepted residual, grammar-level, not fixable in reveal) | **Measured** |
...
| C++ | ✅ 100%³ (Godot), 100%²⁶ (assimp) | ✅ 83.3% (Godot) | ✅ 96.26%⁴⁴ (assimp) | **Measured** |
...
**Import-recall column, re-measured 2026-09-24 (BACK-1458, BACK-1460):** every
re-runnable row reproduces its published figure. A regression from mid-August to
2026-09-23 (BACK-1460: a file with any tree-sitter parse error lost all its imports)
had pulled C, C++, Zig, Python, Go, Kotlin and TSX below them (C/curl 47.04%, C++/Godot
`core/` 55.79%); it is fixed. Sixteen corpus/oracle pairs can no longer be re-run
...
2. **This table covers three signals, not all of reveal's DD output.**
   ...
   ground-truth measurements too — Python (BACK-972): all 10 real
   ...
   TypeScript/JavaScript (BACK-973): `jest.mock`/`vi.mock`/`spyOn`/`fn`
   measured 100% recall and precision against an independent
   TypeScript-compiler-API oracle on two corpora with different mocking
...
**BACK-738** — shape 1 (this annotation-before-function-type case) was
later fixed via ERROR-node recovery in `kotlin.py` and shipped in
...
(BACK-1460 fixed): all re-runnable rows reproduce; 16 cannot be re-run.** From 2026-08-28 (BACK-1214 made `depends://` report
...
- **Reproduce as published:** TypeScript/nest 99.93%, PHP/WordPress 100.00%,
  ...
  Scala/GitBucket 100%.
- **Regressed, then fixed (BACK-1460):** from mid-August to 2026-09-23 a file with any
  tree-sitter ERROR node contributed no imports and no symbols. On 2026-09-23 that read
  Python 92.28%, Go 98.42%, Kotlin 93.13%, TSX/Excalidraw 99.60%, C/Redis 70.32%,
  C/curl 47.04%, C++/Godot `core/` 55.79%, `editor/` 55.34%, `modules/` 83.75%,
  C++/assimp 58.70%, Zig/ghostty 83.57%, Zig/TigerBeetle 78.59%. With the fix (such a
...
they were published: Python 83.5% → 86.1% and TypeScript 76.8% → 90.9% (`env` reads
...
| Rust | Meilisearch `milli` crate (238 files) | reverse | 96.98%/95.27% → **100.00%** | 0 (post-fix) | Turbofish generics on a call/method callee defeated the bare-callee-name extractor's naive last-`::`-split (`size_of::<u32>()` → misparsed callee `<u32>`), plus a minor parenthesized-callee gap (`(f)(args)` captured literally) — **BACK-733**, fixed. **Caveat (2026-09-22):** this `syn::visit` oracle treats macro bodies as opaque token streams, so the 100% says nothing about calls inside macro invocations (`format!`, `vec!`, `assert_eq!`, …). `calls://` was blind to them too until **BACK-1393** (best-effort: an identifier followed by a `(…)` group inside a macro's token tree); a re-run against this oracle will show those edges as false positives, since the oracle cannot see them. Spot-check instead: all 32 functions that left `?uncalled` on meilisearch `index-scheduler`/`milli`/`meilisearch` are called inside a macro, 0 added |
...
| Ruby | Discourse `app/`+`lib/` (1,899 files) | reverse | **22.05%** → **100.00%** (original oracle); **99.80%** (HEAD, stricter oracle that also counts paren-less calls, 20/bucket, 8 of 4,078 edges missed) | 30 (pre-fix) → 0 (post-fix); 2 at HEAD | Ruby's `'call'` node is the same tree-sitter kind Python's plain `call()` uses but a flat receiver/./method/args shape, so `child(0)` returned the receiver for every qualified/self/static call — **BACK-735**, fixed, same class as BACK-734. Two more distinct bugs found in the residual tail while at 99%: `setter`/`operator`-named methods (`def x=`/`def []`) invisible to `--outline` (same class as BACK-638/651/724), and `def Class.method` singleton methods naming themselves after the qualifying constant instead of the method (`_PARAM_LIST_KINDS` missing Ruby's `method_parameters` kind) — both fixed same session |
