...
**For CLI:** `reveal help://quick` loads a compact intent router (~300 tokens). `reveal --agent-help` loads the orientation section (~500 tokens); `reveal help://agent/full` loads the complete reference (~40,000 tokens).
...
reveal --agent-help              # AI agent orientation (~500 tokens; help://agent/full for the rest)
