...
**F6.b — Substring-match false positives** (mitigated 2026-05-05; **F6.b-followup fixed 2026-05-05 tempestuous-frost-0505 / BACK-283**).
Adding bare `'header'` to the http kind caused user wrappers like `printHeader` and `printSubheader` to misclassify as http (substring match is greedy). Initial mitigation removed `header` from the taxonomy with an in-code comment. **F6.b-followup (BACK-283)**: `classify_call` now tokenizes both patterns and callees on `.`, `->`, `::`, and whitespace, then requires the pattern segment sequence to match consecutively as whole segments (sliding-window equality, case-insensitive). `printHeader` no longer matches `header`; `gmail` no longer matches `mail`; `mylog` no longer matches `log`. `header` was re-added to the http taxonomy and bare `header()` calls now classify correctly. 12 new tests in `TestClassifyCallBoundaryMatch`. Full reveal suite: 8613 passed, 0 failures. Unblocks BACK-285 (receiver-shape heuristics now safe to add).
...
Recommendation: pair F6.d with the F6.b-followup (exact-or-boundary matching). Once matching is more precise, Option B becomes safer because false-positive risk drops. Then a small Option C surface (just a list of additional `kind: pattern` strings) gives the escape hatch. **Update 2026-05-05 (tempestuous-frost-0505):** F6.b-followup landed via BACK-283 — segment-boundary matching is now in place, so Option B is now safe to implement (BACK-285).
