...
Discovery (find what reveal can do):
  ...
  reveal --agent-help         Agent orientation (~1,200 tokens; reveal help://agent/full for all)
...
def _add_discovery_options(group) -> None:
    ...
    group.add_argument('--agent-help', action='store_true',
                       help='Agent orientation (~1,200 tokens; reveal help://agent/full for the complete task-pattern reference). Complements --help (raw flag listing).')
...
def _add_code_analysis_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--transitive', action='store_true',
                        help='With --sideeffects or --boundary: follow calls into project-local helper functions (blast-radius/'
                             'agentic-exposure read) instead of only the entry function\'s own body; combine with --depth to '
                             'control how many hops (default 2, max 5) '
