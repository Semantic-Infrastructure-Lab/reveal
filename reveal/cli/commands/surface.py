...
def _scan_surface(path: Path, type_filter: str = '', source_only: bool = False) -> Dict[str, Any]:
    ...
    (py_files, ts_files, java_files, cs_files, php_files, swift_files,
     kt_files, rb_files, go_files) = collected
    ...
    coverage = assess_language_coverage(
        path, {'python', 'typescript', 'tsx', 'java', 'csharp', 'php', 'swift', 'kotlin', 'ruby', 'go'})
...
def _collect_source_files(path: Path, source_only: bool = False):
    """Return (py, ts, java, cs, php, swift, kotlin, ruby, go) file lists for the given path."""
    _EXT_BUCKETS = (
        (frozenset({'.py'}), 0),
        (frozenset({'.ts', '.tsx'}), 1),
        (frozenset({'.java'}), 2),
...
def _render_report(report: Dict[str, Any], top: int = None) -> None:
    ...
    if total == 0:
        if not warning:
            ...
            if lang:
                print("  reveal surface currently supports Python, TypeScript, Java, C#, PHP, Swift, Kotlin, Ruby, and Go.")
