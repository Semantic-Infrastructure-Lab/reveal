...
def _extract_param_types(
    ...
    for c in _children(func_node):
        if _zero_arg(c, 'kind') == 'parameters':
            params = c
            break
    if params is None:
        return None
    ...
    for child in _children(params):
        if _zero_arg(child, 'kind') == 'typed_parameter':
            name_node = next(
                (c for c in _children(child) if _zero_arg(c, 'kind') == 'identifier'), None
            )
            if name_node is None or get_text(name_node) != var_name:
                continue
            type_node = next(
                (c for c in _children(child) if _zero_arg(c, 'kind') == 'type'), None
            )
...
def _parse_type_node(node: Any, get_text: Callable) -> FrozenSet[str]:
    ...
    if _zero_arg(node, 'kind') == 'none':
        return frozenset({'None'})

    if _zero_arg(node, 'kind') == 'generic_type':
        name = get_text(node.child(0))
        params = next(
            (c for c in _children(node) if _zero_arg(c, 'kind') == 'type_parameter'), None
        )
        if params is None:
    ...
    if _zero_arg(node, 'kind') == 'binary_operator':
        # X | Y syntax (PEP 604)
...
def _classify_isinstance_guard(
    ...
    if callee is None or _zero_arg(callee, 'kind') != 'identifier':
        return None
    if get_text(callee) != 'isinstance':
        return None
    args = next((c for c in _children(node) if _zero_arg(c, 'kind') == 'argument_list'), None)
    if args is None:
        return None
    arg_ids = [c for c in _children(args) if _zero_arg(c, 'kind') == 'identifier']
...
def _classify_is_none_guard(node: Any, var_name: str, get_text: Callable) -> Optional[Dict[str, Any]]:
    ...
    if len(children) < 3:
        return None
    subj = children[0]
    if _zero_arg(subj, 'kind') != 'identifier' or get_text(subj) != var_name:
        return None
    op_type = _zero_arg(children[1], 'kind')  # 'is' or 'is not'
    rhs = children[-1]
    if _zero_arg(rhs, 'kind') != 'none':
        return None
    if op_type == 'is':
...
def _classify_guard(
    ...
    if _zero_arg(node, 'kind') == 'not_operator':
        negated = True
        node = next((c for c in _children(node) if _zero_arg(c, 'kind') == 'call'), None)
        if node is None:
            return None
    ...
    if _zero_arg(node, 'kind') == 'comparison_operator' and not negated:
...
def _walk_stmts(
    ...
    for stmt in _children(block_node):
        ...
        elif _zero_arg(stmt, 'kind') == 'assert_statement':
...
def _handle_if(
    ...
    if guard is None:
        ...
        for c in _children(if_node):
            ...
            elif _zero_arg(c, 'kind') in ('elif_clause', 'else_clause'):
                body = next((x for x in _children(c) if _zero_arg(x, 'kind') == 'block'), None)
                if body:
    ...
    if_body = next((c for c in _children(if_node) if _zero_arg(c, 'kind') == 'block'), None)
    if_exits = False
    if if_body:
    ...
    alternatives = [
        c for c in _children(if_node) if _zero_arg(c, 'kind') in ('elif_clause', 'else_clause')
    ]
...
def _walk_alternatives(
    ...
    for alt in alternatives:
        ...
        elif _zero_arg(alt, 'kind') == 'elif_clause':
...
def _handle_assert(
    ...
    cond = next(
        (c for c in _children(assert_node)
         if _zero_arg(c, 'kind') not in ('assert', ',', 'comment')),
        None,
    )
    if cond is None:
        return type_set
...
def _get_body(func_node: Any) -> Optional[Any]:
    return next((c for c in _children(func_node) if _zero_arg(c, 'kind') == 'block'), None)


def _get_if_condition(if_node: Any) -> Optional[Any]:
    skip = {'if', ':', 'block', 'elif_clause', 'else_clause', 'comment'}
    return next((c for c in _children(if_node) if _zero_arg(c, 'kind') not in skip), None)


def _get_elif_condition(elif_node: Any) -> Optional[Any]:
    skip = {'elif', ':', 'block', 'comment'}
    return next((c for c in _children(elif_node) if _zero_arg(c, 'kind') not in skip), None)


def _block_always_exits(block_node: Any) -> bool:
    stmts = [
        c for c in _children(block_node)
        if _zero_arg(c, 'kind') not in ('comment', 'newline', 'indent', 'dedent')
    ]
    return bool(stmts) and _zero_arg(stmts[-1], 'kind') in ('return_statement', 'raise_statement')
