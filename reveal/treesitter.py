...
class TreeSitterAnalyzer(FileAnalyzer):
    ...
    def _extract_language_specific_functions(self) -> List[StructureItem]:
        """Hook for a language whose functions-as-values carry their name on a parent
        node (JS-family `const f = () => {}` and class fields, Lua's
        `name = function(...) ... end`, BACK-758). No-op by default — overridden in
        analyzers/_js_function_values.py and analyzers/lua.py (BACK-915, BACK-1280).
        """
        return []

    def _find_named_function_value(self, name: str):
        """Resolve a bare name to a function-as-value node (a function literal whose name
        lives on its parent), for element lookup and nav-flag lookup
    ...
    def _extract_decorated_functions(self, function_types: List[str]) -> tuple[List[StructureItem], set]:
        ...
        for decorated_node in decorated_nodes:
            ...
            for child in _children(decorated_node):
                if _zero_arg(child, 'kind') in function_types:
                    func_node = child
                elif _zero_arg(child, 'kind') == 'decorator':
    ...
    def _extract_decorated_classes(self, class_types: List[str]) -> tuple[List[StructureItem], set]:
        ...
        for decorated_node in decorated_nodes:
            ...
            for child in _children(decorated_node):
                if _zero_arg(child, 'kind') in class_types:
                    class_node = child
                elif _zero_arg(child, 'kind') == 'decorator':
                    decorators.append(self._get_node_text(child))
        ...
        for child in _children(node):
            if _zero_arg(child, 'kind') == 'base_clause':
                for base_child in _children(child):
                    if _zero_arg(base_child, 'kind') == 'name':
        ...
        for class_type in class_types:
            ...
            for node in nodes:
                ...
                if not name:
                    ...
                    if _zero_arg(node, 'kind') == 'anonymous_class':
    ...
    def _extract_generic_type_base(self, generic_type) -> Optional[str]:
        ...
        for gchild in _children(generic_type):
            if _zero_arg(gchild, 'kind') == 'type_identifier':
    ...
    def _find_nodes_by_type(self, node_type: str) -> List[Any]:
        ...
        if self._node_cache is None:
            ...
            for node in _iter_tree(tree_root(self.tree)):
                cache.setdefault(_zero_arg(node, 'kind'), []).append(node)
    ...
    def _function_end_node(self, node):
        ...
        if _zero_arg(node, 'kind') in (
            ...
            # below never fired, and the final `_zero_arg(sibling, 'kind') ==
            ...
            if sibling is not None and _zero_arg(sibling, 'kind') == 'function_body':
                return sibling
        return node
    ...
    def _name_via_param_adjacent(self, kids) -> Optional[str]:
        ...
        for i, child in enumerate(kids):
            if _zero_arg(child, 'kind') in _PARAM_LIST_KINDS and i > 0:
                prev = kids[i - 1]
    ...
    def _name_via_field_identifier(self, kids) -> Optional[str]:
        ...
        for child in kids:
            if _zero_arg(child, 'kind') == 'field_identifier':
                return self._get_node_text(child)
        return None
    ...
    def _get_node_name(self, node) -> Optional[str]:
        ...
        """
        if _zero_arg(node, 'kind') == 'struct_type':
    ...
    def _find_identifier_in_tree(self, node) -> Optional[str]:
        ...
        # Check current node
        if _zero_arg(node, 'kind') in ('identifier', 'name', 'simple_identifier', 'field_identifier'):
            return self._get_node_text(node)

        # Search children recursively
        for child in _children(node):
            # Skip pointer/reference symbols and parameter lists
            if _zero_arg(child, 'kind') in ('*', '&', 'parameter_list', 'parameters'):
                continue
    ...
    def _get_signature(self, node) -> str:
        ...
        for i, child in enumerate(kids):
            if _zero_arg(child, 'kind') in _PARAM_LIST_KINDS and i > 0 and _zero_arg(kids[i - 1], 'kind') in _NAME_KINDS:
        ...
        if not params_text:
            ...
            for child in kids:
                if _zero_arg(child, 'kind') in _PARAM_LIST_KINDS:
                    params_text = self._get_node_text(child)
                    break

        for child in kids:
            if _zero_arg(child, 'kind') in ('return_type', 'type'):
