...
class TestFileCollectionResultCounts:
    ...
    excluded instead of discarding that information at the point it's known."""

    def test_skipped_gitignore_counted(self, tmp_path):
    ...
    def test_skipped_dirs_counted(self, tmp_path):
        for dirname in ['.git', '__pycache__', 'node_modules']:
            subdir = tmp_path / dirname
            subdir.mkdir()
            (subdir / 'file.py').write_text('# skipped')
...
# ==========================================================================
# Tests for grouped output, generated file detection, and N006
# ==========================================================================

class TestGroupedOutput:
    """Tests for the _print_grouped_detections helper."""
    ...
    def test_no_group_flag_shows_all(self, capsys):
        from reveal.cli.file_checker import _print_grouped_detections
        detections = [self._make_detection("N003", i) for i in range(15)]
        _print_grouped_detections(detections, "test.conf", no_group=True)
        out = capsys.readouterr().out
        assert out.count("N003") == 15

    def test_high_count_rule_is_collapsed(self, capsys):
        from reveal.cli.file_checker import _print_grouped_detections
        detections = [self._make_detection("N003", i) for i in range(15)]
        _print_grouped_detections(detections, "test.conf", no_group=False)
        out = capsys.readouterr().out
        # First occurrence shown once (in the detection line) + once in the summary note
        assert out.count("test.conf:") == 1   # only one detection line printed
        assert "+14 more N003" in out

    def test_low_count_rule_shown_in_full(self, capsys):
        from reveal.cli.file_checker import _print_grouped_detections
        detections = [self._make_detection("N004", i) for i in range(3)]
        _print_grouped_detections(detections, "test.conf", no_group=False)
        out = capsys.readouterr().out
    ...
    def test_mixed_rules_collapses_only_large_group(self, capsys):
        from reveal.cli.file_checker import _print_grouped_detections
        detections = (
            [self._make_detection("N003", i) for i in range(12)]
            + [self._make_detection("N004", 100 + i) for i in range(3)]
        )
        _print_grouped_detections(detections, "test.conf", no_group=False)
        out = capsys.readouterr().out
        assert "+11 more N003" in out
        assert out.count("N004") == 3

    def test_threshold_boundary_9_not_collapsed(self, capsys):
        from reveal.cli.file_checker import _print_grouped_detections, _GROUP_THRESHOLD
        detections = [self._make_detection("N003", i) for i in range(_GROUP_THRESHOLD - 1)]
        _print_grouped_detections(detections, "test.conf", no_group=False)
        out = capsys.readouterr().out
        assert "more N003" not in out

    def test_threshold_boundary_10_collapsed(self, capsys):
        from reveal.cli.file_checker import _print_grouped_detections, _GROUP_THRESHOLD
        detections = [self._make_detection("N003", i) for i in range(_GROUP_THRESHOLD)]
        _print_grouped_detections(detections, "test.conf", no_group=False)
        out = capsys.readouterr().out
...
class TestGeneratedFileDetection:
    """Tests for _is_generated_file helper."""

    def test_reveal_generated_marker(self):
        from reveal.checks import _is_generated_file
        content = "# reveal: generated\nserver { listen 80; }"
        assert _is_generated_file(content) is True

    def test_auto_generated_comment(self):
        from reveal.checks import _is_generated_file
        content = "# This file is automatically generated by cPanel\nserver {}"
        assert _is_generated_file(content) is True

    def test_generated_by_comment(self):
        from reveal.checks import _is_generated_file
        content = "# Generated by certbot\nserver {}"
        assert _is_generated_file(content) is True

    def test_at_generated_comment(self):
        from reveal.checks import _is_generated_file
        content = "# @generated\nserver {}"
        assert _is_generated_file(content) is True

    def test_normal_nginx_config_not_generated(self):
        from reveal.checks import _is_generated_file
        content = "# Production nginx config\nserver { listen 80; }"
        assert _is_generated_file(content) is False

    def test_marker_beyond_15_lines_ignored(self):
        from reveal.checks import _is_generated_file
        lines = ["# line {}\n".format(i) for i in range(20)]
        lines[18] = "# Generated by something\n"
        assert _is_generated_file(''.join(lines)) is False

    def test_case_insensitive(self):
        from reveal.checks import _is_generated_file
        assert _is_generated_file("# AUTO-GENERATED\nserver {}") is True
        assert _is_generated_file("# Reveal: Generated\nserver {}") is True


class TestN006Rule:
    """Tests for N006 send_timeout vs client_max_body_size cross-check."""

    def _run(self, content):
        from reveal.rules.infrastructure.N006 import N006
        rule = N006()
        return rule.check("nginx.conf", {}, content)

    def test_dangerous_combination_flagged(self):
        content = "http {\n    send_timeout 30s;\n    client_max_body_size 200m;\n}"
        findings = self._run(content)
        assert len(findings) == 1
        assert "send_timeout" in findings[0].message
        assert "N006" == findings[0].rule_code

    def test_safe_timeout_no_flag(self):
        content = "http {\n    send_timeout 300s;\n    client_max_body_size 200m;\n}"
        assert self._run(content) == []

    def test_small_body_no_flag(self):
        content = "http {\n    send_timeout 10s;\n    client_max_body_size 1m;\n}"
        assert self._run(content) == []

    def test_proxy_read_timeout_also_checked(self):
        content = "http {\n    proxy_read_timeout 20s;\n    client_max_body_size 50m;\n}"
        findings = self._run(content)
        assert len(findings) == 1
        assert "proxy_read_timeout" in findings[0].message

    def test_both_timeouts_flagged(self):
        content = (
            "http {\n"
            "    send_timeout 30s;\n"
    ...
    def test_missing_body_size_no_flag(self):
        content = "http {\n    send_timeout 30s;\n}"
        assert self._run(content) == []
    ...
    def test_suggestion_present(self):
        ...
        assert "60s" in findings[0].suggestion

    def test_body_size_10m_boundary_no_flag(self):
        """Exactly at threshold — should NOT fire (must be strictly greater)."""
        content = "http {\n    send_timeout 30s;\n    client_max_body_size 10m;\n}"
        assert self._run(content) == []

    def test_body_size_11m_fires(self):
        content = "http {\n    send_timeout 30s;\n    client_max_body_size 11m;\n}"
        assert len(self._run(content)) == 1

    def test_detection_severity_is_high(self):
        from reveal.rules.base import Severity
        content = "http {\n    send_timeout 30s;\n    client_max_body_size 200m;\n}"
        findings = self._run(content)
