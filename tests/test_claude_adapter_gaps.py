...
class TestClaudeSchemaParamParity:
    ...
    # 'role' removed (consolidation: ?role=user|assistant folded into ?raw on
    # /prompts and /messages, see schema.py's _SCHEMA_NOTES entry on the
    # /user + /assistant element removal).
    HANDLER_PARAMS = {'summary', 'errors', 'tools', 'contains', 'raw',
...
class TestGetHumanPrompts:
    ...
    def test_excludes_task_notification(self):
        # Real-corpus finding: async Agent-tool completions are delivered as an
        # ordinary type=user/content=str record whose text is a literal
        # <task-notification>...</task-notification> body — before this fix,
        # _is_real_prompt's bare isinstance(content, str) check couldn't tell that
        # apart from a real human prompt. Confirmed on 3 real sessions with Agent
        # tool usage: ~30% of "prompts" were these leaked notifications.
        from reveal.adapters.claude.analysis.messages import get_human_prompts
        messages = [
            _user_msg('do the review'),
            _user_msg('<task-notification>\n<task-id>abc123</task-id>\n<status>completed</status>\n</task-notification>'),
            _user_msg('thanks, next step'),
        ]
        result = get_human_prompts(messages, 'sess', self._cb())
        assert result['message_count'] == 2
        texts = [b['text'] for m in result['messages'] for b in m['content'] if b.get('type') == 'text']
        assert texts == ['do the review', 'thanks, next step']

    def test_excludes_bash_mode_wrapper_tags(self):
        # Bash-mode (`!command`) turns inject <local-command-caveat>,
        # <command-name>, <command-message>, <local-command-stdout> — none typed
        # by a human, all previously leaking into /prompts as bare-string content.
        from reveal.adapters.claude.analysis.messages import get_human_prompts
        messages = [
            _user_msg('<local-command-caveat>Caveat: the messages below were generated by the user while running local commands.</local-command-caveat>'),
            _user_msg('<command-name>/exit</command-name>\n<command-message>exit</command-message>'),
            _user_msg('<local-command-stdout>Bye!</local-command-stdout>'),
            _user_msg('real prompt after bash mode'),
        ]
        result = get_human_prompts(messages, 'sess', self._cb())
        assert result['message_count'] == 1
        texts = [b['text'] for m in result['messages'] for b in m['content'] if b.get('type') == 'text']
        assert texts == ['real prompt after bash mode']

    def test_does_not_exclude_real_prompt_pasting_html(self):
        # The generic "^<[a-z]" tag-prefix convention used elsewhere (title
        # extraction) is deliberately NOT reused here: real sessions confirmed
        # genuine human prompts starting with <p>, <html>, <script>, etc. (pasted
        # markup while debugging). Only a whitelist of known harness-injected tag
        # names should be excluded, never a generic "starts with a tag" heuristic.
        from reveal.adapters.claude.analysis.messages import get_human_prompts
        messages = [_user_msg('<script>alert(1)</script> is this XSS-safe?')]
        result = get_human_prompts(messages, 'sess', self._cb())
        assert result['message_count'] == 1
